import { isValidHex, isValidOpaqueHex } from './color-validation';
...
describe('isValidHex', () => {
  it('returns true for valid hex', () => {
    ...
    hexValues.forEach((hex) => {
      expect(isValidHex(hex), `Expected ${hex} to be valid`).toBe(true);
    });
  });
  it('returns false for non-hex strings', () => {
    const hexValues = [
    ...
    hexValues.forEach((hex) => {
      expect(isValidHex(hex), `Expected ${hex} to be invalid`).toBe(false);
    });
  });
});
